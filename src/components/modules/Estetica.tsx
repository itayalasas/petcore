...
const SERVICE_TYPES = [
  ...
  { value: 'bathing', label: 'Bano' },
  { value: 'nail_trim', label: 'Corte de unas' },
  { value: 'haircut', label: 'Corte de pelo' },
  { value: 'spa', label: 'Spa' },
...
export default function Estetica() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setServices(servicesData.filter(s => s.service_type === 'grooming'));
      setStaff(staffData);
    ...
    },
    {
      ...
      render: (_: any, row: PetService) => (
        <div>
          ...
          <p className="text-xs text-gray-500">{SERVICE_TYPES.find(t => t.value === row.service_type)?.label}</p>
        </div>
      ),
    },
    {
  ...
  if (loading) {
    return <LoadingSpinner message="Cargando estetica..." />;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Estetica</h1>
          <p className="text-gray-500">Gestion de servicios de peluqueria y bano</p>
        </div>
        <button
          onClick={() => {
            resetForm();
